...
#
  ...
  #  cd ..
    ...
    cd ..
    decode=$(steps/nnet/decode.sh --nj 4 exp/tri_nn_$i\_$j/graph data/test_asr_all exp/tri_nn_$i\_$j\_nnet/decode_test_asrall)
    Elapsed_Decode=$(( $SECONDS - $Start_Decode ))
    cd my-local
    echo "Running WER for tri=$i $j"
    score=$(./score_words.sh ../data/test_asr_all ../exp/tri_$totgauss\_$num_leaves/graph ../exp/tri_nn_$i\_$j\_nnet/decode_test_asrall)
    wer=$(cat ../exp/tri_nn_$i\_$j\_nnet/decode_test/scoring_kaldi/best_wer|grep -Po "(?<=WER )[0-9]*\.*[0-9]*")
    echo "Training time:$Elapsed_Train">>../exp/tri_nn_$i\_$j\_nnet/run_log_asrall
    echo "Make graph time:$Elapsed_Graph">>../exp/tri_nn_$i\_$j\_nnet/run_log_asrall
    echo "Decode time:$Elapsed_Decode">>../exp/tri_nn_$i\_$j\_nnet/run_log_asrall
    echo "WER:$wer" >>../exp/tri_nn_$i\_$j\_nnet/run_log_asrall
#   done
# done
