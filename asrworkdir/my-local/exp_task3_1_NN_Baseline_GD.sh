...
for gender in  male
do
decode=$(steps/nnet/decode.sh --nj 4  --skip-scoring true exp/tri_nn_$i\_$j/graph data/my_test_$gender exp/tri_nn_$i\_$j\_nnet/decode_test_$gender)
cd my-local
echo "Running WER for gender=$gender"
score=$(./score_words.sh ../data/my_test_$gender ../exp/tri_15000_2000/graph ../exp/tri_nn_$i\_$j\_nnet/decode_test_$gender)
wer=$(cat ../exp/tri_nn_$i\_$j\_nnet/decode_test_$gender/scoring_kaldi/best_wer|grep -Po "(?<=WER )[0-9]*\.*[0-9]*")
echo "Training time:$Elapsed_Train">>../exp/tri_nn_$i\_$j\_nnet/run_log_$gender
echo "Make graph time:$Elapsed_Graph">>../exp/tri_nn_$i\_$j\_nnet/run_log_$gender
echo "Decode time:$Elapsed_Decode">>../exp/tri_nn_$i\_$j\_nnet/run_log_$gender
echo "WER:$wer" >>../exp/tri_nn_$i\_$j\_nnet/run_log_$gender
cd ..
done
